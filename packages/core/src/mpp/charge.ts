 ...
 */
export async function runMppChargeChecks(options: MppChargeCheckOptions): Promise<CheckResult[]> {
    ...
    return [
      {
        ...
        name: "Charge Settlement On-Chain",
        pass: false,
        detail: `2xx response but no valid Payment-Receipt header: ${(err as Error).message}`,
      },
    ];
  }
  ...
  const horizon = new Horizon.Server(
    options.network === "stellar:pubnet"
    ...
    return [
      {
        ...
        name: "Charge Settlement On-Chain",
        pass: false,
        detail: `Payment-Receipt referenced tx ${reference}, but Horizon lookup failed: ${(err as Error).message}`,
      },
    ];
  }
}
