 ...
 */
async function checkResponseStatus(target: string): Promise<{ res: Response; result: CheckResult }> {
 ...
 */
function checkPaymentHeaderPresent(res: Response): { headerValue: string | null; result: CheckResult } {
 ...
 */
function checkHeaderDecodable(headerValue: string | null): { payload: any | null; result: CheckResult } {
  if (headerValue === null) {
    return {
      payload: null,
  ...
  try {
    ...
    return {
      payload: null,
      result: { id: "X402-03", name: "Header Payload Decodable", pass: false, detail: `Failed to decode/parse: ${(err as Error).message}` },
    };
  }
}
 ...
 */
function checkRequiredFields(payload: any | null): CheckResult {
  if (payload === null) {
...
export async function runX402ReadChecks(options: X402SimulatorOptions): Promise<CheckResult[]> {
