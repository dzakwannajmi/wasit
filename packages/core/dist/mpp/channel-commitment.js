 ...
 */
export function commitmentKeypairFromHex(secretHex) {
    const seed = Buffer.from(secretHex, "hex");
    if (seed.length !== COMMITMENT_SEED_BYTES) {
        throw new CommitmentSimulationError(`Commitment secret must decode to ${COMMITMENT_SEED_BYTES} bytes, got ${seed.length}.`);
    }
 ...
 */
export function nextValidCumulative(previousCumulative, requestedAmount) {
    ...
    try {
        return await Promise.race([
            ...
            new Promise((_resolve, reject) => {
                timer = setTimeout(() => reject(new CommitmentSimulationError(`${label} timed out after ${timeoutMs}ms.`)), timeoutMs);
            }),
        ]);
    }
    finally {
 ...
 */
export async function prepareCommitmentBytes(params) {
    const { channelContract, amount, networkPassphrase, rpcUrl, simulationTimeoutMs = DEFAULT_SIMULATION_TIMEOUT_MS, } = params;
    const server = new rpc.Server(rpcUrl);
    const contract = new Contract(channelContract);
    const operation = contract.call("prepare_commitment", nativeToScVal(amount, { type: "i128" }));
    const source = new Account(SIMULATION_SOURCE_ACCOUNT, "0");
    const transaction = new TransactionBuilder(source, {
        fee: SIMULATION_FEE,
        networkPassphrase,
    ...
    let simulation;
    try {
        simulation = await withTimeout(server.simulateTransaction(transaction), simulationTimeoutMs, "prepare_commitment simulation");
    }
    catch (error) {
        if (error instanceof CommitmentSimulationError)
            throw error;
        throw new CommitmentSimulationError(`prepare_commitment simulation failed at transport level: ${error instanceof Error ? error.message : String(error)}`, error);
    }
    if (rpc.Api.isSimulationError(simulation)) {
        throw new CommitmentSimulationError(`prepare_commitment simulation returned an error: ${simulation.error}`);
    }
    // Checked BEFORE isSimulationSuccess: the restore response type structurally
    ...
    if (rpc.Api.isSimulationRestore(simulation)) {
        throw new CommitmentSimulationError("Channel contract requires a storage restore before it can be simulated. " +
            "Extend the contract's TTL before running channel checks.");
    }
    if (!rpc.Api.isSimulationSuccess(simulation)) {
        throw new CommitmentSimulationError("prepare_commitment simulation returned an unrecognised response shape.");
    }
    const returnValue = simulation.result?.retval;
    if (!returnValue) {
 ...
 */
export async function signChannelCommitment(params) {
    ...
    if (signature.length !== ED25519_SIGNATURE_BYTES) {
        throw new CommitmentSimulationError(`Expected a ${ED25519_SIGNATURE_BYTES}-byte signature, got ${signature.length}.`);
    }
