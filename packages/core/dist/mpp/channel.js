 ...
 */
export async function runMppChannelDeployChecks(options) {
    let state;
    try {
    ...
    if (state.refundWaitingPeriod !== options.expected.refundWaitingPeriod) {
        mismatches.push(`refundWaitingPeriod: expected ${options.expected.refundWaitingPeriod}, got ${state.refundWaitingPeriod}`);
    }
...
function failure(id, name, detail) {
 ...
 */
export async function runMppChannelOrderingCheck(options) {
    const id = "MPP-11";
    ...
    try {
        ...
        if (accepted.status !== 200) {
            return failure(id, name, `Setup failed: a correctly advancing commitment (${baseline}) was not accepted — ` +
        ...
        const equal = await submitCredential(target, (await buildChannelCredential({
        ...
        if (equal.status !== 402) {
            return failure(id, name, `A commitment equal to the current cumulative (${baseline}) was not rejected — ` +
            ...
            return [
                {
                    ...
                    name,
                    pass: true,
                    detail: `Non-advancing commitment correctly rejected (HTTP 402). ` +
        ...
        const short = await submitCredential(target, (await buildChannelCredential({
        ...
        if (short.status !== 402) {
            return failure(id, name, `A commitment that advances but under-covers the request price ` +
        ...
        return [
            {
                ...
                name,
                pass: true,
                detail: `Both ordering rules enforced: a commitment equal to the cumulative ` +
 ...
 */
export async function runMppChannelReplayCheck(options) {
    const id = "MPP-12";
    ...
    try {
        ...
        if (first.status !== 200) {
            return failure(id, name, `Setup failed: a valid commitment (${amount}) was not accepted — ` +
                `HTTP ${first.status}: ${first.body}`);
        }
        const replay = await submitCredential(target, credential);
        if (replay.status !== 402) {
            return failure(id, name, `An identical credential was accepted twice — expected HTTP 402 on replay, ` +
 ...
 */
export async function runMppChannelCommitmentReplayCheck(options) {
    const id = "MPP-14";
    ...
    try {
        ...
        const first = await submitCredential(target, serializeChannelCredential({ challenge: opening, amount, signature }));
        if (first.status !== 200) {
            return failure(id, name, `Setup failed: a valid commitment (${amount}) was not accepted — ` +
                `HTTP ${first.status}: ${first.body}`);
        }
        ...
        const replay = await submitCredential(target, serializeChannelCredential({ challenge: fresh, amount, signature }));
        if (replay.status !== 402) {
            return failure(id, name, `A captured commitment (${amount}) was redeemed a second time against a ` +
        ...
        return [
            {
                ...
                name,
                pass: true,
                detail: `Captured commitment (${amount}) correctly rejected when re-presented ` +
...
export async function runMppChannelCloseCheck(options) {
    const id = "MPP-13";
    const name = "Close Settlement";
    const { target, commitmentSecretHex, network, rpcUrl, expectedChannel, allowDestructive = false, closePollAttempts = 6, closePollDelayMs = 2_000, } = options;
    if (!allowDestructive) {
        return [
            skippedDestructive(id, name, "closing settles the channel on-chain and permanently ends it. " +
                "Re-run with destructive checks enabled, against a channel you own."),
        ];
    }
    if (!expectedChannel) {
        return failure(id, name, "Destructive checks are enabled but no expected channel was named. " +
    ...
    try {
        ...
        if (channel !== expectedChannel) {
            return failure(id, name, `Refusing to close: ${target} bills through channel ${channel}, but the ` +
        ...
        const readState = () => getChannelState({
            channel,
        ...
        if (before.closeEffectiveAtLedger !== null) {
            return failure(id, name, `Channel ${channel} is already closing or closed ` +
        ...
        if (closed.status !== 200) {
            return failure(id, name, `Close credential (${amount}) was not accepted — expected HTTP 200, ` +
                `got ${closed.status}: ${closed.body}`);
        }
        let after;
        for (let attempt = 0; attempt < closePollAttempts; attempt += 1) {
            after = await readState();
            if (after.closeEffectiveAtLedger !== null)
                break;
            await sleep(closePollDelayMs);
        }
        if (!after || after.closeEffectiveAtLedger === null) {
            return failure(id, name, `Server reported the close succeeded, but on-chain state still shows ` +
        ...
        if (delta !== amount) {
            return failure(id, name, `Channel closed (closeEffectiveAtLedger=${after.closeEffectiveAtLedger}), ` +
        ...
        return [
            {
                ...
                pass: true,
                destructive: true,
                detail: `Close settled on-chain: balance fell by exactly the committed ${amount} ` +
