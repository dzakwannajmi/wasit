...
import { z } from "zod";
const DESTRUCTIVE_ENABLED = process.env.WASIT_ALLOW_DESTRUCTIVE === "1" ||
    process.argv.includes("--allow-destructive");
/** Locates docs/CHECKS.md by walking up from this file. */
function locateChecksCatalogue() {
...
function present(results) {
    ...
    const outcome = counts.exitCode === 0
        ? "conformant"
        : counts.exitCode === 1
            ? "non-conformant"
            : "no-verdict";
    ...
    const summaryLine = `${counts.passed} passed, ${counts.failed} failed, ` +
        `${counts.errored} could not run, ${counts.skipped} skipped.`;
    const caveat = counts.errored > 0
...
function missingKey(variable) {
    return {
        content: [
            {
                type: "text",
                text: `${variable} is not set in this server's environment. Secrets are read ` +
...
if (catalogue) {
    server.registerResource("checks-catalogue", "wasit://checks", {
        title: "Wasit Check Catalogue",
        description: "Every check Wasit runs, with its spec reference and exact pass criteria. " +
...
server.registerTool("wasit_x402_test", {
    title: "Test x402 compliance",
    description: "Runs the x402 conformance checks (X402-01..07) against a running service. " +
...
const channelInputShape = {
    ...
    channel: z
        .string()
        .optional()
        .describe("Assert which channel the target bills through. The target's own challenge " +
...
async function runChannel(input, allowDestructive, destructiveChannel) {
...
server.registerTool("wasit_mpp_channel_test", {
    title: "Test MPP channel-mode compliance",
    description: "Runs the non-destructive MPP channel-mode checks (MPP-10, 11, 12, 14) " +
    ...
    if (!process.env.COMMITMENT_SECRET_HEX)
        return missingKey("COMMITMENT_SECRET_HEX");
    return runChannel(input, false);
});
if (DESTRUCTIVE_ENABLED) {
    server.registerTool("wasit_mpp_channel_test_with_close", {
        title: "Test MPP channel-mode compliance, including close settlement",
        description: "Runs the full MPP channel-mode suite INCLUDING MPP-13 (Close Settlement). " +
